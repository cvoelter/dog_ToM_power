---
title: "Study 2 - Power simulation"
...
two conditions: sound-cue and sound control
between-subject design
age and sex counterbalanced 
DV: unseen food: 1; seen food: 0
...
cue.per <- c(0.75, 0.8) # performance in sound-cue condition
cont.per <- c(0.5) # performance in sound-control condition

start.data <- data.frame(subj.id)
xdata=data.frame(expand.grid(condition=c("cont", "cue"), sex=c("F", "M")))
...
for (i in 1:nrow(all.res)) {
  ...
  coefs <- c(
    "(Intercept)" = qlogis(all.res[i, "cont.per"]),#performance in sound-control condition
    "conditioncue" = qlogis(all.res[i, "cue.per"])-qlogis(all.res[i, "cont.per"]),#performance in sound-cue condition
    "sexM" = 0,
    "z.age" = 0
  )
...
xdata$choice <- rbinom(n = nrow(xdata), size = 1, prob = exp(LP) / (1 + exp(LP)))
  ...
  # store results:
  all.res[i, c("icpt", "conditioncue", "z.age", "sexM")] <- coef(full$value)
...
save.image("study2_simulation_N76.RData")
```
...
```{r echo=FALSE}
p.con.power <- ggplot(data = lrt.data2, aes(x= as.factor(cue.per),y = proportion.sign.lrt.con, fill=as.factor(cont.per))) +
  ...
  labs(fill = "sound-control condition", y="Power", x= "sound-cue condition") +
  theme_few()#+
  #theme(legend.position="none")
p.con.power

ggsave(p.con.power, filename = "./graphics/study2_choice_simulation_binomial_glm.png", scale = 0.8, height = 5, width = 7)
```